    ...
    if (current_player == PLAYER) {
      ...
      if (!ai_first_move_done) {
        ...
        if (mv.first != -1) {
          ...
          cout << "【AI第一手】落子: 行 " << (mv.first + 1)
               << ", 列 " << (mv.second + 1)
               << ", 耗时 " << fixed << setprecision(2) << spent << "s" << endl;
        }
